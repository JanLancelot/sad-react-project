...
function Events() {
  ...
  const handleDepartmentChange = (e) => {
    ...
    if (selectedDepartment === '') {
      ...
      const filtered = events.filter((event) => event.department === selectedDepartment);
  ...
  const handleEventFilterChange = (e) => {
    ...
    if (filterValue === 'all') {
      ...
      const filtered = events.filter((event) => new Date(event.date.split('/').reverse().join('-')) < new Date());
      setFilteredEvents(filtered);
          ...
          <thead>
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                Event Name
                <button onClick={handleSortByScore} className="ml-2 text-xs font-medium text-blue-500">
                  Sort by Score
                </button>
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredEvents.map((event) => (
              <tr key={event.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">{event.numberOfAttendees}</td>
                <td className="px-6 py-4 whitespace-nowrap">{event.averageScore}</td>
              </tr>
            ))}
          </tbody>
        </table>
