...
function PendingMeetings() {
  ...
  const handleApprove = async (meetingId) => {
    try {
      ...
      const meetingData = pendingMeetings.find((meeting) => meeting.id === meetingId);
      const newMeetingData = { ...meetingData, pending: false };

      const newMeetingRef = await addDoc(collection(db, 'meetings'), newMeetingData);
  ...
  return (
    <Layout>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {adRole?.toLowerCase() === 'admin' || adRole?.toLowerCase() === 'osas' ? (
          ...
          <div className="p-4 rounded-md bg-gray-100 text-gray-700">
            You do not have permission to view or approve pending events.
          </div>
        )}
        <ToastContainer />
      </div>
